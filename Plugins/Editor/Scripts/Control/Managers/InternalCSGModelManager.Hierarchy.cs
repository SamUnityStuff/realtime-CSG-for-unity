...
namespace RealtimeCSG
{
    internal partial class InternalCSGModelManager
    {
        ...
        internal const int BrushCheckChunk = 1000;
        internal static int BrushCheckPos = 0;

        // HACK: Because CheckTransformChanged gets called every frame anyway
        public static void CheckTransformChanged(bool nothing = false) {
            SlicedCheckTransformChanged();
        }
        public static void SlicedCheckTransformChanged() {
            ...
            // 1. If we're not gonna hit all our brushes, check selected transforms for active selection
            if(BrushCheckChunk < Brushes.Count)
            {
                GameObject[] gameObjects = Selection.gameObjects;
                for (int i = 0; i < gameObjects.Length; i++) {
                    if (gameObjects[i].TryGetComponent(out CSGBrush brush)) {
                        CheckSingleBrushTransform(brush);
                    }
                }
            }
            ...
            if (Brushes.Count > 0) {
                ...
                for (int sliceIdx = 0; sliceIdx < sliceSize; sliceIdx++) {
                    ...
                    BrushCheckPos++;
                    BrushCheckPos %= Brushes.Count;

                    var brush = Brushes[BrushCheckPos];
                    CheckSingleBrushTransform(brush);
                }
            }
        }
        static void CheckSingleBrushTransform(CSGBrush brush) {
        ...
        // TODO: Move to UnityEngine.ObjectDispatcher?
        // ORIGINAL
        public static void OLD_SINGLEPASS_CheckTransformChanged(bool checkAllBrushes = false) {
            if (External == null) {
                return;
            }
